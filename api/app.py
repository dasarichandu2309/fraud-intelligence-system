...
def login(data: LoginRequest):
    ...
    if hash_password(data.password) != db_password:
...
def predict(data: PredictRequest, user=Depends(get_current_user)):
    ...
    new_device = 1 if cur.fetchone()[0] == 0 else 0
    ...
    if prob > 0.8:
        ...
        risk_score += 30
        reasons.append("Moderate ML fraud probability")

    if amount > avg_amount * 20:
        risk_score += 60
        reasons.append("Extreme amount spike")
    elif amount > avg_amount * 10:
        risk_score += 50
        reasons.append("Very high transaction")
    elif amount > avg_amount * 5:
        risk_score += 40
        reasons.append("High transaction")
    elif amount > avg_amount * 2:
        risk_score += 25
        reasons.append("Moderate spike")
    ...
    if amount > 1000000:
        ...
        reasons.append("Amount exceeds safe banking limit")
...
@app.post("/blacklist")
def add_blacklist(user_id: int, reason: str, user=Depends(get_current_user)):
    conn = get_connection()
    cur = conn.cursor()
